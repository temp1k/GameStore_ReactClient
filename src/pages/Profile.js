                ...
                <Card className={"mt-2 w-100"} bg={"light"} text={"dark"}>
                    ...
                    <Card.Footer style={{paddingLeft: "5rem", paddingRight: "5rem"}}>
                        <Row className={"justify-content-end"}>
                            <Button variant={"secondary"} style={{marginRight: "1rem"}} onClick={switchShowModalProfileData}>Изменить данные</Button>
                    ...
                    <Container>
                        {profileData.purchases.map((item, index) =>
                            <Card className={"ml-auto mr-auto mb-2 d-flex w-75"} key={item.id}>
                                <Row>
                                    <Col xs={3}>
                                        <ImageComponent fileName={item.codeGameNavigation.game.photo} style={{height: 100}} />
                                    </Col>
                                    <Col xs={5}>
                                        <Card.Body style={{padding: "7px 0"}}>
                                            <Card.Title>{item.codeGameNavigation.game.fullName}</Card.Title>
                                            <Card.Text className={"text-left"}><b>Платформа:</b> {item.codeGameNavigation.platform.fullName}</Card.Text>
                                        </Card.Body>
                                    </Col>
                                    <Col xs={4} className={"d-flex align-items-end justify-content-end"}>
                                        <Button size={"sm"} variant={"warning"} className={"mr-2 mb-2"}>Скопировать код</Button>
                                    </Col>
                                </Row>
                            </Card>
                        )}
