...
import {changeGame, createGame, getGameById} from "../../http/gameAPI";
import {formatDate} from "../../utils/formatDate";
import {Button, Form, Modal} from "react-bootstrap";
import ImageComponent from "../ImageComponent";
import {observer} from "mobx-react-lite";
import {deleteDeveloper, getDeveloperById, postDeveloper, putDeveloper} from "../../http/developersAPI";

let defaultDeveloper = {
    fullName: '',
    shortName: ''
}
const DeveloperModal = observer(({show, onHide, title, selectedId, setSelectedId = 0, callback}) => {
    const {
        values,
        setValues,
        errors,
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            if (validate(values, errors, setErrors)) {
                ...
                if (selectedId !== 0) {
                    putDeveloper(formData, selectedId)
                        .then(data => {
                            onHide();
                            callback();
                        }).catch(err => console.log(err))
                } else {
                    postDeveloper(formData)
                        .then(data => {
                            onHide();
                            callback();
                        })
